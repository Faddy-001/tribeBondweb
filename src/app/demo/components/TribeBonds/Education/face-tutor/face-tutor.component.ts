...
export class FaceTutorComponent {
  ...
  navigateToAdd(){
    this.router.navigateByUrl(`/tribe/facetutor/${this.idParam}/add-tutor`);
  
  }
  navigateToDetail(id:number): void {
    this.router.navigateByUrl(`/tribe/facetutor/${this.idParam}/tutor-detail/${id}`);
  }
  ...
  getAllEductionDisplay() {
      this.auth.getAllEducation().subscribe(
          (res: any) => {
              ...
              this.allData = res
              console.log(this.allData.data[2].entities);
              
              this.allData.data[2].entities.forEach((data: any) => {
                  console.log(data);
                  this.entities = data;
