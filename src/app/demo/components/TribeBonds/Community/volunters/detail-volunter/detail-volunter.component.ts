...
@Component({
  selector: 'app-detail-volunter',

  templateUrl: './detail-volunter.component.html',
  styleUrl: './detail-volunter.component.scss'
})
export class DetailVolunterComponent {
  images!: any[];
...
navigateEdit(id:number): void {
    this.router.navigateByUrl(`/tribe/volunter/edit-volunter/${this.idParam}`);
  }
...
AddReview() {
    ...
    const dataValue = {
        "volunteerEventId": this.idParam,
        "review": this.reviewText
    }
    console.log(dataValue);

    // You can then perform any logic you need with the review text
    this.auth.addVolunteerReview(dataValue).subscribe(
        (result) => {
