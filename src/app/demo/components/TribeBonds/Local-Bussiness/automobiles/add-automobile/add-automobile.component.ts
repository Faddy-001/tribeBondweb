...
userDataString:any
  ...
  Submit(value: any) {
   ...
   if (this.addAutomobileForm.valid) {
    ...
    this.auth.addAutomobile(this.formData).subscribe(
      (result: any) => {
        this.automibileResult = result;
        console.log(this.automibileResult.message);

        this.toastr.success(this.automibileResult.message);

        this.router.navigate([`/tribe/automobileList`]);
        this.addAutomobileForm.reset();
